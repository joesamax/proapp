...
function build_page_object( $dic ) {
	...
	$childobject['ml_link']   = trailingslashit( apply_filters( 'ml_site_root', get_site_url() ) ) . 'ml-api/v1/post/?post_id=' . $dic->ID;
...
foreach ( $pages as $p ) {
	...
	if ( $p->post_title != null && $p->ID != null ) {
		...
		$page['ml_link']   = trailingslashit( apply_filters( 'ml_site_root', get_site_url() ) ) . 'ml-api/v1/post/?post_id=' . $p->ID;
