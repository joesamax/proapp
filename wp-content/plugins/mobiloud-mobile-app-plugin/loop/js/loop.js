...
function createItemContent( article ) {
	...
	thumb = ons.createElement( '<figure class="article-list__thumb" data-bg="' + article.featured_image["big-thumb"].url + '"></figure>' );
	wrap.append(thumb);

	// Article text
	var content = ons.createElement( '<div class="article-list__content"><h2>' + article.title + '</h2><p>' + article.excerpt + '</p></div>' );
	wrap.append(content);

	// Article meta
	if (article.categories[0] != null) {
		cat = '<a tappable data-ml-cat-id="' + article.categories[0].cat_id + '">' + article.categories[0].name + '</a> ';
	} else {
		cat = '';
	}
	var meta = ons.createElement( '<div class="article-list__meta">' +
		cat +
		'<span class="author">' + article.author.name + '</span>' +
		'</div>' );

	// Article element
	var listItem = ons.createElement( '<ons-list-item class="article-list__article" tappable data-ml-post-id="' + article.post_id + '" data-ml-post-permalink="' + article.permalink + '" data-ml-opening-method="' + article.opening_method + '"></ons-list-item>' );
	...
	for( var i = 0; i < posts; i++ ) {
		...
		article.addEventListener( 'click', function () {
			...
			if ( this.getAttribute('data-ml-opening-method') == 'internal' ) {
				...
				nativeFunctions.handlePost( this.getAttribute('data-ml-post-id') );
			}
		} );
...
async function getNewArticles(url, offset, params) {
	...
	return new Promise(function (resolve, reject) {
		getJSON( url + 'ml-api/v1/posts?' + params + '&offset=' + offset,
			function( err, data ) {
				if ( err != null ) {
...
function BackgroundNode( { node, loadedClassName } ) {
	let src = node.getAttribute('data-bg');
	let show = ( onComplete ) => {
		...
		getJSON( url + 'ml-api/v1/posts' + params + '&offset=' + offset,
			function( err, data ) {
				if ( err != null ) {
					...
					mlFirstData[ 'posts' ] = newPosts.posts.concat( mlFirstData.posts );
					...
					setTimeout( function() {
						document.querySelectorAll( '.article-list__article.new-item' ).forEach( e => {
							if ( e.classList.contains( 'new-item' ) ) {
								e.classList.remove( 'new-item' );

								e.style.height = 'auto';
								var height = e.clientHeight + "px";
								e.style.height = '0px';
...
function mlScrollList() {
	if( document.querySelector('.page__content').scrollTop == 0 ) {
		...
		mlCheckNewArticlesInterval = setInterval( function() {
			fetchNewlyPublishedArticles();
		}, 60000 );
	} else {
		clearInterval(mlCheckNewArticlesInterval);
...
document.querySelector('.page__content').addEventListener( 'scroll', function() {
