...
class MLPostsController {
	...
	public function get_final_posts( $posts, $offset, $taxonomy, $post_count, $cache_on, $image_format = 1 ) {
		...
		foreach ( $posts as $post ) {
			...
			$post_id      = $post->ID;
			$cached_posts = $cache_on ? $this->get_cached( $post_id, $image_format ) : null;
			...
			if ( $cache_on ) {
				$key = $ml_cache->post_cache_key( $post_id, $image_format );
	 ...
	 * @return array
	 */
	private function get_cached( $post_id, $image_format ) {
		$cached = false;
