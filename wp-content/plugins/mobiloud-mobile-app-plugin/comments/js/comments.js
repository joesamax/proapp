...
function submitCommentForm() {
	...
	if ( comment != '' ) {
		...
		postComment( comment ).then(function ( data ) {
			...
			if ( data.id ) {
				switch( data.status ) {
					case "approved":
						...
						document.querySelector('#comment-list > h4').remove();
						break;
					case "unapproved":
						textarea.value = '';
...
async function process(email, callback) {
	...
	var avatarData = 'action=process_comments&do=avatar&email=' + email;
...
textarea.addEventListener( "focus", checkUserDetails );
