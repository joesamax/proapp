jQuery(document).ready(function() {
	...
	jQuery( '#ml_tabbed_navigation_enabled' ).on(
		'change',
		function() {
			jQuery( '#tabbed_navigation_settings' ).toggleClass( 'active' );
		}
	);

	jQuery( 'input.ml-tab-label' ).on(
		'keyup',
		function() {
			var tab_id = jQuery( this ).parents( ".tabnav-content" ).attr( 'id' ).split('tabnav-');
			var target = jQuery( '#tabnav-tab-' + tab_id[1] + ' div' );
			target.text( jQuery( this ).val() );
		}
	);
