...
// var_dump($this->homepreset_settings);
 ...
 <table class="form-table">
 	<tr>
 		...
 		<td>
 			<select id="nh_homescreentype" name="<?php echo $this->homepreset_settings_key; ?>[homescreentype]" class="nh-floatleft">
            	...
            	<option value="3" <?php if ($this->homepreset_settings['homescreentype']=='3') {
     echo 'selected="selected"';
 } ?>><?php _e('Categories', 'nh-ynaa'); ?></option>
                <option value="1" <?php if ($this->homepreset_settings['homescreentype']=='1') {
     echo ' selected="selected"';
 } ?>><?php _e('Posts', 'nh-ynaa'); ?></option>
                <option value="2" <?php if ($this->homepreset_settings['homescreentype']=='2') {
     echo ' selected="selected"';
 } ?>><?php _e('Pages', 'nh-ynaa'); ?></option>
            </select>
           <?php echo '<div class="helptext padding5">'.(__('Select your startscreen view for your app.', 'nh-ynaa')).'</div>'; ?>
           </td>
 	</tr>

 	<tr id="nh_posttype-tr" style="<?php if ($this->homepreset_settings['homescreentype']!='1') {
     echo 'display:none;';
 } ?>">
        ...
        foreach ($post_types as $post_type) {
            if (!in_array($post_type, array( 'attachment', 'revision', 'nav_menu_item' ))) {
                if (!empty($this->homepreset_settings['posttype'][$post_type])) {
 	...
 	<tr class="nh_sorttype-tr" style="<?php if (isset($this->homepreset_settings['homescreentype']) && ($this->homepreset_settings['homescreentype']==0 || $this->homepreset_settings['homescreentype']==3)) {
            echo 'display:none;';
        }  ?>">
 		...
 		<td>
 			<select id="nh_sorttype" name="<?php echo $this->homepreset_settings_key; ?>[sorttype]" class="nh-floatleft">
            	<option value="date-desc" <?php if ($this->homepreset_settings['sorttype']=='date-desc') {
            echo ' selected="selected"';
        } ?>><?php _e('Recent posts', 'nh-ynaa'); ?></option>
                <option value="date-asc" <?php if ($this->homepreset_settings['sorttype']=='date-asc') {
            echo ' selected="selected"';
        } ?>><?php _e('Oldest posts', 'nh-ynaa'); ?></option>
                <option value="alpha-asc" <?php if ($this->homepreset_settings['sorttype']=='alpha-asc') {
            echo ' selected="selected"';
...
<div id="nav-menus-frame" style="<?php if (isset($this->homepreset_settings['homescreentype']) && $this->homepreset_settings['homescreentype']!=0) {
            echo 'display:none;';
        }?>">
		...
		<div class="accordion-container" id="side-sortables">
			<ul class="outer-border">
				...
				<div class="accordion-section-content ">
                    ...
                    do_action('nh_add_app_extras_homepresets');
            ...
            <?php
                ...
                do_action('nh_add_accordion_homepresets');
                ?>
		</ul><!-- .outer-border -->
							...
							<div id="menu-accordion">
								...
								<ul id="menu-to-edit" class="menu nav-menus-php nh-homepreset-ul">
                                    ...
                                    if ($this->homepreset_settings['items'] && !empty($this->homepreset_settings['items'])) {
                                        ...
                                        $i = 0;
                                        foreach ($this->appmenus_pre as $ar) {
                                            $types[] = $ar['type'];
                                        }
                                        ...
                                        foreach ($menuitems as $v=>$ar) {
                                            ...
                                            if (($ar['img'])) {
                                                if (($ar['img'])) {
                                                    echo  'style="background-image:url(\''.($ar['img']).'\'); background-color:'.$this->general_settings['c1'].';"';
                                                }
                                            }
                                            if ($ar['type']=='cat') {
                                                if (!empty($this->categories_settings[$ar['id']])) {
                                                    if (($this->categories_settings[$ar['id']]['img']) && ($this->categories_settings[$ar['id']]['usecatimg'])) {
                                                        echo 'style="background-image:url(\'' . ($this->categories_settings[$ar['id']]['img']) . '\');"';
                                                    } else {
                                                        $items = ($this->nh_ynaa_articles($ar['id'], 1));
                                                        if ($items['articles']['items'] && !empty($items['articles']['items'][0]['thumb'])) {
                                                            echo 'style="background-image:url(\'' . ($items['articles']['items'][0]['thumb']) . '\');"';
                                                        } else {
                                                    ...
                                                    /*elseif($ar['type']=='pushCenter' || $ar['type']=='carFinder'){
                                                        ...
                                                        echo 'style="background-image:url(\''.($this->nh_getthumblepic($ar['id'])).'\'); background-color:'.$this->general_settings['c1'].';"';
                                                    }
                                            echo ' >';

                                            echo '<div class="ttitle'.$deactivated_class.'" id="hptitle'.$v.'div" title="'.$deactivated_cat_title.'">'.$ar['title'].'</div>';
                                                    //if($ar['type']=='cat' || in_array($ar['type'],$types) || true ) {
                                                 ?>

                                                    <div class="setdefaultcatpic" <?php if (($ar['type']!='webview' && $ar['type']!='carFinder' && $ar['type']!='pushCenter') && false) {
                                                     echo ' style="display:none;"';
                                                 } ?>><a id="upload_image_button<?php echo $v; ?>" class="upload_image_button" href="#" name="<?php echo $this->homepreset_settings_key; ?>_items_<?php echo $v; ?>_img"><?php _e('Set default image', 'nh-ynaa'); ?></a></div>
                                            ...
                                            if ($ar['hidetitle']) {
                                                ...
                                                <div><label class="tooltip " title="<?php _e('Hide the title from the startscreen', 'nh-ynaa'); ?>" for="hphide<?php echo $v; ?>"><?php _e('Hide title from startscreen', 'nh-ynaa'); ?> </label><input id="hphide<?php echo $v; ?>" value="1" name="<?php echo $this->homepreset_settings_key; ?>[items][<?php echo $v; ?>][hidetitle]" type="checkbox" style="width: 16px;" <?php echo $checked; ?>></div>
                                                <?php

                                                if (isset($this->appmenus_pre[$ar['id']]['extrafields'])) {
                                                    foreach ($this->appmenus_pre[$ar['id']]['extrafields'] as $k2=>$extrafield) {
                                                        ?>
														<div class="h30"><input
																		type="<?php echo $extrafield['type']; ?>"
																		value="<?php echo $ar[$k2]; ?>"
																		name="<?php echo $this->homepreset_settings_key; ?>[items][<?php echo $v; ?>][<?php echo $k2; ?>]">
															</div>
														<?php
                                            ...
                                            if (isset($ar['customfilter'])) {
                                                echo '<input type="hidden" value="'.$ar['customfilter'].'" name="'.$this->homepreset_settings_key.'[items]['.$v.'][customfilter]" id="'.$this->homepreset_settings_key.'_items_'.$v.'_customfilter">';
                                            }
